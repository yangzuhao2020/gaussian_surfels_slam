...
def training(config: dict, arg=None):
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, total_num_frames)):
        ...
        if time_idx > 0:
            gaussians.c(time_idx, forward_prop=config['tracking']['forward_prop'])
            # 这里表示 相机的位姿。
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']:
            ...
            while True:
                ...
                loss = new_get_loss(params, # 所有（渲染出的）的高斯点和相机位姿
                                    variables, # 辅助初始变量。
                                    iter_time_idx, # 帧数情况。
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            for iter in range(num_iters_mapping):
                ...
                # Loss for current frame
                loss, variables, losses = new_get_loss(params, 
                                                        iter_data, 
                                                        variables, 
                                                        iter_time_idx, 
...
def new_get_loss(curr_data, iter_time_idx, sil_thres, guassians, use_sil_for_loss=True,
                 tracking=False, do_ba=False, ignore_outlier_depth_loss=True):
    if tracking:
        transformed_pts = transform_to_frame(iter_time_idx, 
                                         gaussians_grad=False, 
                                         camera_grad=True)  # 仅优化相机位姿
