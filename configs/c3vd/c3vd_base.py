...
tracking_iters = 15 # 相机位姿势优化 每帧优化15次。
# tracking_iters = 5
mapping_iters = 25 # 高斯点的优化。
...
config = dict(
    ...
    tracking=dict(
        use_gt_poses=False, # Use GT Poses for Tracking 是否使用真实位姿
        forward_prop=True, # Forward Propagate Poses 是否用于前向传播
        num_iters=tracking_iters, # 相机位姿势优化 每帧优化15次。
        use_sil_for_loss=True, # 是否使用轮廓损失。
        sil_thres=0.99,
        use_l1=True,
        ignore_outlier_depth_loss=False, # 是否忽略异常深度点
        loss_weights=dict(
            im=0.5, # 图像重投影损失的权重。
            depth=1.0, # 深度损失的权重
        ),
        lrs=dict(
            means3D=0.0, # 3D 高斯点云的均值位置，不优化
            rgb_colors=0.0,
            unnorm_rotations=0.0,
            logit_opacities=0.0, 
            log_scales=0.0, # 3d 高斯尺寸学习率。
            cam_unnorm_rots=0.002, # 相机旋转学习率
            cam_trans=0.005, # 相机平移学习率
        ),
    ),
    mapping=dict(
